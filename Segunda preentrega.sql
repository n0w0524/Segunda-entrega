...
CREATE TABLE `movie_library`.`acciones` (
...
DELIMITER $$
USE `movie_library`$$
CREATE DEFINER = CURRENT_USER TRIGGER `movie_library`.`titulo_obligatorio` BEFORE INSERT ON `peliculas` FOR EACH ROW
BEGIN
...
DELIMITER $$
USE `movie_library`$$
CREATE DEFINER = CURRENT_USER TRIGGER `movie_library`.`log_peliculas` AFTER INSERT ON `peliculas` FOR EACH ROW
BEGIN
...
values 
("Breve Encuentro", "1:26:00", 1945, "Al encontrarse con un extraño en una estación de tren, una mujer siente la tentación de engañar a su marido."),
...
("Antes del Amanecer", "1:41:00", 1995, "Un joven y una mujer se conocen en un tren en Europa y terminan pasando una noche juntos en Viena. Desafortunadamente, ambos saben que esta será probablemente la única noche que estén juntos."),
